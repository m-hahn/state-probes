...
#        print("============================tgt_state")
        ...
        for q in range(tgt_state["labels"].size()[0]):
         ...
         #       print(q, r, tokenizer.towords(tgt_state["all_states_input_ids"][q,r]), ["?", "T", "F"][int(tgt_state["labels"][q,r])], tokenizer.towords(inputs["input_ids"][q]))
                ...
                input_here = torch.cat([inputs["input_ids"][q], to_device(torch.LongTensor([tokenizer._separator])), tgt_state["all_states_input_ids"][q,r]][::-1], dim=0)
#                print("PREPARED", tokenizer.towords(tgt_state["all_states_input_ids"][q,r]))
#                input_here = torch.cat([tgt_state["all_states_input_ids"][q,r]][::-1], dim=0)
 #               print("PREPARED", tokenizer.towords(input_here))
...
#        print(labels)
        optimizer.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(parameters(), max_norm=15)
        optimizer.step()
        num_updates += 1
