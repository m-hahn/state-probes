...
from transformers import BartForConditionalGeneration, T5ForConditionalGeneration, BartForSequenceClassification,  GPT2LMHeadModel
...
model_fp = 'gpt2'
tokenizer = GPT2TokenizerFast.from_pretrained(model_fp, local_files_only=args.local_files_only)
...
print(tokenizer.encode("No"))
print(tokenizer.encode("NO"))
print(tokenizer.decode(torch.LongTensor([43335, 15285])))
#quit()
        ...
        for q in range(tgt_state["labels"].size()[0]):
            ...
            for r in range(tgt_state["labels"].size()[1]):
              ...
              if int(tgt_state["labels"][q,r]) > 0:
                LABEL_TOKENIZED = [15, 15285, 43335][int(tgt_state["labels"][q,r])]
         #       print(q, r, tokenizer.towords(tgt_state["all_states_input_ids"][q,r]), ["?", "T", "F"][int(tgt_state["labels"][q,r])], tokenizer.towords(inputs["input_ids"][q]))
                ...
                input_here = torch.cat([inputs["input_ids"][q], to_device(torch.LongTensor([1])), tgt_state["all_states_input_ids"][q,r], to_device(torch.LongTensor([LABEL_TOKENIZED]))], dim=0)
...
#                print(q, r, tokenizer.decode(tgt_state["all_states_input_ids"][q,r]), ["?", "T", "F"][int(tgt_state["labels"][q,r])], tokenizer.decode(inputs["input_ids"][q]))
                ...
                Y = tuple(inputs["input_ids"][q].cpu().numpy().tolist())
                Z = int(tgt_state["labels"][q,r])
                if (X,Y) in appeared_in_train:
                   print("Warning: appeared in traINING SET")
                   continue
                if (X,Y) in input_to_responses:
                    if input_to_responses[(X,Y)] == Z:
                      correct += 1
   ...
   #               batch_input = []
    #              batch_labels = []
                  ...
                  for y in range(len(labels)):
                      text_tokenized = to_device(torch.LongTensor([x for x in inputs_and_qs[y].long().cpu().numpy().tolist() if x != 15]))
                      inputs_and_qs[y] = torch.cat([text_tokenized, to_device((0 + torch.zeros(max_length - text_tokenized.size()[0]).long())).long()], dim=0)
                  if j % 10 == 0:
                     print(max_length)
                  if max_length > 290:
                       continue
...
#attention_mask=inputs['attention_mask'],
                  if True:
                    #print(inputs_and_qs)
                    return_dict = model(
                      input_ids=inputs_and_qs, labels=inputs_and_qs, return_dict=True #,  num_labels=2
                    )
                    #quit()
#                  except RuntimeError:
 #                    print("OOM")
  #                   print(max_length)
   #                  continue
                  lang_loss, dec_output = return_dict.loss, return_dict.logits
